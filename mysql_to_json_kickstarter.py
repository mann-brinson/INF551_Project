#GOAL: Convert the kickstarter.sql into json, so it can be uploaded to Firebase
import os
...
def select_from_table(table):
    ...
    Currently assumes we are selecting from KICKSTARTER_SAMP database
...
def main():
    ...
    tables = ['project', 'creator', 'comments']
    primary_keys = ['project_id', 'creator_id', 'comment_id']
    index = 'index'
    root = 'https://inf551kickstarter.firebaseio.com/'
    suffix = '.json'
    inv_index_full = dict()
